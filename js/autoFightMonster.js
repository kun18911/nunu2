...
function testValid() { // kiểm tra xem bàn cờ có đi đc nước nào ko?  
    ...
    for (var r = 0; r < hàng; r++) {
        for (var c = 0; c < cột ; c++) {
            var k = board[r][c].id.split("-"), a = +k[0], b = +k[1]
            changeSrcTest(a + 1 < hàng, 'test', 1, 0)// hoán đổi chính nó và viên dưới nó
            changeSrcTest(b + 1 < cột, 'test', 0, 1)// hoán đổi chính nó và viên bên phải
...
function changeSrcTest(condition1, condition2, y, z, array, num) { 
    if (condition1) {
        // hàm hoán đổi src
        function mimi() { 
            var img1 = board[i][e].src, img2 = board[i + y][e + z].src
            // 1. đổi src img cho nhau
            [img1.src, img2.src] = [img2.src, img1.src]; 
            // 2. đổi symbolClass img cho nhau
            [img1.dataset.symbolClass, img2.dataset.symbolClass] = 
                        [img2.dataset.symbolClass, img1.dataset.symbolClass];
            // 3. đổi ảnh Filter cho nhau
            changeFilter(img1, img2); 
        }; 
        ...
        if (checkValid()) {
            if (condition2 == 1) {
                ...
                if (testV[0] == true) {
                    pi.a++;
                    array.push([img1, img2, testV[1], testV[2]])
                }
            }
        };
...
function fakeTurn(num) {
    ...
    for (var r = 0; r < hàng; r++) {
        for (var c = 0; c < cột; c++) { /**xuống*/ /**phải */
            var k = board[r][c].id.split("-"); a = +k[0]; b = +k[1]; 
            changeSrcTest(a + 1 < hàng, undefined, 1, 0, auto_array, num); 
            changeSrcTest(b + 1 < cột, undefined, 0, 1, auto_array, num); 
        }
    } 
...
function auto_Fight1() { //var mảng_Auto = [auto_4x, auto_3x]
    ...
    var y = Math.floor(Math.random() * mảng_True.length), nn = mảng_True[y] 
    // lấy đc random id     
    //   true = [0,2,3]  3length 0  ||  true false true true 
    var boomChess = [], branch = [], n = 0 //a[1] lấy all mảng|
    arraySelected.forEach((i, index) => { 
    ...
    if (nn == 0) { mimi(mảng_Đi_5_Hướng[0][1])/**4 */ } 
    else if (nn == 1) { mimi(mảng_Đi_5_Hướng[1][1]) }
}
function auto_Fight() {
